...
contentRoutes.get("/vocabulary",async(c)=>{const u=c.get("user");const page=asNumber(c.req.query("page"),1,1);const limit=asNumber(c.req.query("limit"),100,1,200);const category=String(c.req.query("category")||"").trim();const query=String(c.req.query("q")||"").trim();let where="(v.user_id IS NULL OR v.user_id=?)";const params:unknown[]=[u.id];if(category){where+=" AND v.category=?";params.push(category)}if(query){const pattern=likePattern(query);where+=" AND (v.word LIKE ? OR v.meaning LIKE ?)";params.push(pattern,pattern)}const totalRow=await first<{count:number}>(c.env.DB,`SELECT COUNT(*) count FROM vocabulary v WHERE ${where}`,params);const rows=await all(c.env.DB,`SELECT v.*,COALESCE(vr.stability,0) stability,COALESCE(vr.difficulty,0.5) difficulty,vr.due_date,CASE WHEN vr.stability>=7 THEN 100 WHEN vr.stability IS NULL THEN v.mastery ELSE MIN(99,CAST(vr.stability*12 AS INTEGER)) END mastery FROM vocabulary v LEFT JOIN vocabulary_reviews vr ON vr.word_id=v.id AND vr.user_id=? WHERE ${where} ORDER BY lower(v.word) LIMIT ? OFFSET ?`,[u.id,...params,limit,(page-1)*limit]);const total=Number(totalRow?.count||0);return c.json({vocabulary:rows,total,page,limit,pages:Math.max(1,Math.ceil(total/limit)),query,category})});
